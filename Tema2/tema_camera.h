...
namespace implemented
{
    class Camera_Tema
    {
     public:
        ...
        {
            // Translates the Camera_Tema using the `dir` vector computed from
            // `forward`. Movement will always keep the Camera_Tema at the same
        ...
        {
            // TODO(student): Translate the Camera_Tema using the `forward` vector.
        ...
        {
            // TODO(student): Translate the Camera_Tema using the `up` vector.
        ...
        void TranslateRight(float distance)
        {
            ...
            // You need to translate the Camera_Tema using the `right` vector.
            // Usually, however, translation using Camera_Tema's `right` vector
            // is not very useful, because if the Camera_Tema is rotated around the
            // `forward` vector, then the translation on the `right` direction
            // will have an undesired effect, more precisely, the Camera_Tema will
        ...
        void RotateFirstPerson_OZ(float angle)
        {
            ...
            //forward = glm::normalize(glm::vec3(glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(forward, 1)));
            up = glm::cross(right, forward);
        }

        void RotateThirdPerson_OX(float angle)
        {
            // TODO(student): Rotate the Camera_Tema in third-person mode around
